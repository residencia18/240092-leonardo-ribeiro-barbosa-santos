...
public class UserControllerV2 {
		...
		@GetMapping(value = UserControllerV2.ID)
		...
		public ResponseEntity<UserDTO> create(@RequestBody UserDTO obj) {
			URI uri = ServletUriComponentsBuilder
					.fromCurrentRequest().path(UserControllerV2.ID).buildAndExpand(service.create(obj).getId()).toUri();
			return ResponseEntity.created(uri).build();

		}

		 @PutMapping(value = UserControllerV2.ID)
		 ...
		 @DeleteMapping(value = UserControllerV2.ID)
