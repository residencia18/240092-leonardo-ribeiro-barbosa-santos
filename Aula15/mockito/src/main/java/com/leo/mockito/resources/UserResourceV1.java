...
public class UserResourceV1 {
	...
	@GetMapping(value = UserResourceV1.ID)
	...
	public ResponseEntity<UserDTO> create(@RequestBody UserDTO obj) {
		URI uri = ServletUriComponentsBuilder
				.fromCurrentRequest().path(UserResourceV1.ID).buildAndExpand(service.create(obj).getId()).toUri();
		return ResponseEntity.created(uri).build();

	}

	 @PutMapping(value = UserResourceV1.ID)
	 ...
	 @DeleteMapping(value = UserResourceV1.ID)
